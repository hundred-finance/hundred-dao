...
def test_gauge_integral(accounts, chain, mock_lp_token, reward_policy_maker, gauge_v4_1, gauge_controller):
    ...
    gauge_controller.add_gauge(gauge_v4_1.address, 0, 10 ** 18, {"from": alice})
    ...
    def update_integral():
        ...
        checkpoint = t1
        checkpoint_supply = gauge_v4_1.totalSupply()
        checkpoint_balance = gauge_v4_1.balanceOf(alice)
    ...
    for i in range(40):
        ...
        if is_withdraw:
            amount = randrange(1, gauge_v4_1.balanceOf(bob) + 1)
            gauge_v4_1.withdraw(amount, {"from": bob})
            update_integral()
            bob_staked -= amount
        else:
            amount = randrange(1, mock_lp_token.balanceOf(bob) // 10 + 1)
            mock_lp_token.approve(gauge_v4_1.address, amount, {"from": bob})
            gauge_v4_1.deposit(amount, {"from": bob})
            update_integral()
            bob_staked += amount

        if is_alice:
            # For Alice
            is_withdraw_alice = (gauge_v4_1.balanceOf(alice) > 0) * (random() < 0.5)
            print("Alice", "withdraws" if is_withdraw_alice else "deposits")

            if is_withdraw_alice:
                amount_alice = randrange(1, gauge_v4_1.balanceOf(alice) // 10 + 1)
                gauge_v4_1.withdraw(amount_alice, {"from": alice})
                update_integral()
                alice_staked -= amount_alice
            else:
                amount_alice = randrange(1, mock_lp_token.balanceOf(alice) + 1)
                mock_lp_token.approve(gauge_v4_1.address, amount_alice, {"from": alice})
                gauge_v4_1.deposit(amount_alice, {"from": alice})
                update_integral()
                alice_staked += amount_alice
        ...
        if random() < 0.5:
            gauge_v4_1.user_checkpoint(alice, {"from": alice})
        if random() < 0.5:
            gauge_v4_1.user_checkpoint(bob, {"from": bob})

        assert gauge_v4_1.balanceOf(alice) == alice_staked
        assert gauge_v4_1.balanceOf(bob) == bob_staked
        assert gauge_v4_1.totalSupply() == alice_staked + bob_staked
        ...
        gauge_v4_1.user_checkpoint(alice, {"from": alice})
        update_integral()
        print(i, dt / 86400, integral, gauge_v4_1.integrate_fraction(alice))
        assert approx(gauge_v4_1.integrate_fraction(alice), integral, 1e-15)
...
def test_mining_with_votelock(
    ...
    mock_lp_token.approve(gauge_v4_1.address, MAX_UINT256, {"from": bob})
    ...
    gauge_v4_1.deposit(10 ** 21, {"from": bob})
    ...
    rewards_alice = gauge_v4_1.integrate_fraction(alice)
    rewards_bob = gauge_v4_1.integrate_fraction(bob)

...
def setup(accounts, gauge_controller, gauge_v4, mock_lp_token):
...
def test_sender_balance_decreases(accounts, gauge_v4):
    ...
    gauge_v4.transfer(accounts[1], amount, {"from": accounts[0]})

    assert gauge_v4.balanceOf(accounts[0]) == sender_balance - amount


def test_receiver_balance_increases(accounts, gauge_v4):
    receiver_balance = gauge_v4.balanceOf(accounts[1])
    amount = gauge_v4.balanceOf(accounts[0]) // 4

    gauge_v4.transfer(accounts[1], amount, {"from": accounts[0]})

    assert gauge_v4.balanceOf(accounts[1]) == receiver_balance + amount
...
def test_returns_true(accounts, gauge_v4):
    amount = gauge_v4.balanceOf(accounts[0])
    tx = gauge_v4.transfer(accounts[1], amount, {"from": accounts[0]})
...
def test_transfer_zero_tokens(accounts, gauge_v4):
    ...
    gauge_v4.transfer(accounts[1], 0, {"from": accounts[0]})

    assert gauge_v4.balanceOf(accounts[0]) == sender_balance
    assert gauge_v4.balanceOf(accounts[1]) == receiver_balance
...
def test_transfer_to_self(accounts, gauge_v4):
    ...
    gauge_v4.transfer(accounts[0], amount, {"from": accounts[0]})
...
def test_insufficient_balance(accounts, gauge_v4):
    ...
    with brownie.reverts():
        gauge_v4.transfer(accounts[1], balance + 1, {"from": accounts[0]})

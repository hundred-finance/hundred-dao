...
def test_no_approval_needed(alice, bob, gauge_v4, mock_lp_token):
    mock_lp_token.approve(gauge_v4, 2 ** 256 - 1, {"from": alice})
    gauge_v4.deposit(100_000, bob, {"from": alice})

    assert gauge_v4.balanceOf(bob) == 100_000


def test_deposit_for_and_claim_rewards(
    alice, bob, chain, gauge_v4, mock_lp_token, reward_contract, coin_reward
):
    mock_lp_token.approve(gauge_v4, 2 ** 256 - 1, {"from": alice})
    gauge_v4.deposit(LP_AMOUNT, bob, {"from": alice})
    ...
    gauge_v4.deposit(LP_AMOUNT, bob, True, {"from": alice})

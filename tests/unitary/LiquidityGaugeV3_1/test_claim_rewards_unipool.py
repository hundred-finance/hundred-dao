...
def initial_setup(
    ...
    gauge_controller.add_gauge(gauge_v3_1, 0, 0, {"from": alice})
...
def test_claim_one_lp(bob, chain, gauge_v3_1, coin_reward):
    chain.sleep(WEEK)
...
def test_claim_for_other(bob, charlie, chain, gauge_v3_1, coin_reward):
    chain.sleep(WEEK)
...
def test_claim_for_other_no_reward(bob, charlie, chain, gauge_v3_1, coin_reward):
    chain.sleep(WEEK)
...
def test_claim_two_lp(alice, bob, chain, gauge_v3_1, mock_lp_token, coin_reward, no_call_coverage):
    ...
    # Calculate rewards
    claimable_rewards = [
        gauge_v3_1.claimable_reward_write.call(acc, coin_reward, {"from": acc})
...
def test_claimable_rewards_write_retrieves_rewards(
    alice, chain, gauge_v3_1, mock_lp_token, coin_reward, no_call_coverage
):
    ...
    before = coin_reward.balanceOf(gauge_v3_1)
    gauge_v3_1.claimable_reward_write(alice, coin_reward, {"from": alice})
    assert before < coin_reward.balanceOf(gauge_v3_1)

...
def test_claim_one_lp(bob, chain, gauge_v3_1, coin_a, coin_b):
    chain.sleep(WEEK)
...
def test_claim_updates_claimed_reward(bob, chain, gauge_v3_1, coin_a, coin_b):
    chain.sleep(WEEK)
    ...
    for coin in (coin_a, coin_b):
        ...
        assert reward <= REWARD
        assert approx(REWARD, reward, 1.001 / WEEK)  # ganache-cli jitter of 1 s
        assert gauge_v3_1.claimed_reward(bob, coin) == reward


def test_claim_for_other(bob, charlie, chain, gauge_v3_1, coin_a, coin_b):
    chain.sleep(WEEK)

    gauge_v3_1.withdraw(LP_AMOUNT, {"from": bob})
    gauge_v3_1.claim_rewards(bob, {"from": charlie})
...
def test_claim_for_other_no_reward(bob, charlie, chain, gauge_v3_1, coin_a, coin_b):
    chain.sleep(WEEK)
    gauge_v3_1.claim_rewards(charlie, {"from": bob})
...
def test_claim_two_lp(
    alice, bob, chain, gauge_v3_1, mock_lp_token, coin_a, coin_b, reward_contract, no_call_coverage
):
    ...
    for acct in (alice, bob):
        gauge_v3_1.claim_rewards({"from": acct})

    for coin in (coin_a, coin_b):
        # Calculate rewards
        assert coin.balanceOf(bob) > coin.balanceOf(alice) > 0
        assert coin.balanceOf(gauge_v3_1) == 0


def test_claim_duration(bob, chain, gauge_v3_1, coin_a, coin_b):
    chain.sleep(86400)
    gauge_v3_1.claim_rewards({"from": bob})

    claim_time = gauge_v3_1.last_claim()
    ...
    assert math.isclose(gauge_v3_1.last_claim(), claim_time, abs_tol=1)
    assert claimed == [i.balanceOf(bob) for i in (coin_a, coin_b)]

    chain.sleep(1801)
    gauge_v3_1.claim_rewards({"from": bob})

    assert math.isclose(gauge_v3_1.last_claim(), chain[-1].timestamp, abs_tol=1)
    assert chain[-1].timestamp > claim_time
    assert coin_a.balanceOf(bob) > claimed[0]
    assert coin_b.balanceOf(bob) > claimed[1]


def test_claim_set_alt_receiver(bob, charlie, chain, gauge_v3_1, coin_a, coin_b):
    chain.sleep(WEEK)

    gauge_v3_1.claim_rewards(bob, charlie, {"from": bob})
...
def test_claim_for_other_changing_receiver_reverts(bob, charlie, chain, gauge_v3_1):
    chain.sleep(WEEK)

    gauge_v3_1.withdraw(LP_AMOUNT, {"from": bob})
    with brownie.reverts("dev: cannot redirect when claiming for another user"):
        gauge_v3_1.claim_rewards(bob, charlie, {"from": charlie})
